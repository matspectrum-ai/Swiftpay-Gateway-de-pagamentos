...
A27 is DB-only. It may `CREATE OR REPLACE` only `_a23_resolve_payment_link_pix_attempt` and replace the five `pg_catalog.coalesce(...)` calls in its success-resolution validation with ordinary `coalesce(...)` expressions.
