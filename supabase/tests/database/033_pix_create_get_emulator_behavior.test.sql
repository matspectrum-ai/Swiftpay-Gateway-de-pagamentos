...
begin
    ...
    execute 'reset role';
    insert into pg_temp.a2_results values (p_case_name, v_result, v_error_state, v_error_message);
end;
$$;
...
begin
    select (result -> 'payment' ->> 'id')::uuid,
           (result -> 'providerAttempt' ->> 'id')::uuid
      into v_payment_id, v_attempt_id
      from pg_temp.a2_results
     where case_name = p_prepare_case;
...
begin
    ...
    if p_token_override is not null then
        v_execution_token := p_token_override;
    else
        select (result ->> 'executionToken')::uuid
          into v_execution_token
          from pg_temp.a2_results
         where case_name = p_claim_case;
    end if;
...
begin
    ...
    execute 'set local role swiftpay_api';
    begin
        select app.get_api_payment(p_merchant_id, p_environment, v_payment_id) into v_result;
    exception when others then
        get stacked diagnostics v_error_state = returned_sqlstate, v_error_message = message_text;
    end;
    execute 'reset role';
...
select is(
    ...
    'A2 first prepare persists canonical in-progress idempotency identity');
select ok(
...
select is(
    ...
    'A2 Payment persists complete zero-fee pricing and routing snapshot');
select ok(
...
select ok(
    ...
    'A2 ProviderAttempt is prepared against exactly sandbox emulator account');
select is(
    (select count(*)::text || '|' ||
...
-- Prepared replay and conflict.
call pg_temp.a2_capture_prepare(
    'success_replay_prepared',
    '80000000-0000-0000-0000-000000000001'::uuid,
    'sandbox', 'idem-a2-db-success', repeat('a', 64),
    '{"method":"pix","amount":10101,"currency":"BRL","description":"Pedido A2 DB","externalId":"order-a2-db-001","pixExpirationMinutes":30,"customerName":"Cliente A2","customerEmail":"cliente-a2@example.test"}'::jsonb
);
...
select is(
    (select count(*)::text || '|' || (select count(*) from app.provider_attempts)::text from app.payments),
    '1|1', 'A2 prepared replay creates no second Payment or ProviderAttempt');

call pg_temp.a2_capture_prepare(
    'success_conflict',
    '80000000-0000-0000-0000-000000000001'::uuid,
    'sandbox', 'idem-a2-db-success', repeat('b', 64),
...
select is((select error_state from a2_results where case_name = 'success_conflict'), null,
    'A2 idempotency conflict is returned as normal result');
...
-- DB boundary is sandbox-only and active-merchant-only.
call pg_temp.a2_capture_prepare(
    'production_forbidden',
    '80000000-0000-0000-0000-000000000001'::uuid,
    'production', 'idem-a2-db-production', repeat('c', 64),
...
call pg_temp.a2_capture_prepare(
    ...
    'sandbox', 'idem-a2-db-suspended', repeat('d', 64),
    '{"method":"pix","amount":100,"currency":"BRL","pixExpirationMinutes":30}'::jsonb
);
select ok((select error_state is not null from a2_results where case_name = 'suspended_forbidden'),
    'A2 trusted prepare rejects suspended merchant');

-- Atomic claim.
call pg_temp.a2_capture_claim('success_claim_first', 'success_first');
...
select ok((select (result ->> 'executionToken')::uuid is not null from a2_results where case_name = 'success_claim_first'),
    'A2 winning claim returns UUID execution token');
select ok(
    (select state = 'executing'
         and execution_token = (select (result ->> 'executionToken')::uuid from a2_results where case_name = 'success_claim_first')
         and started_at is not null
         and lease_expires_at between clock_timestamp() + interval '29 seconds' and clock_timestamp() + interval '31 seconds'
       from app.provider_attempts limit 1),
    'A2 winning claim atomically owns thirty-second executing lease');

call pg_temp.a2_capture_claim('success_claim_second', 'success_first');
...
select is((select result ->> 'claimed' from a2_results where case_name = 'success_claim_second'), 'false',
    'A2 second claim cannot own same executing attempt');
...
-- Merchant/environment-scoped GET.
call pg_temp.a2_capture_get('get_owner_creating', '80000000-0000-0000-0000-000000000001'::uuid, 'sandbox', 'success_first');
select ok(
    (select error_state is null and result ->> 'status' = 'creating' and result -> 'pix' = 'null'::jsonb
...
call pg_temp.a2_capture_get('get_foreign', '80000000-0000-0000-0000-000000000002'::uuid, 'sandbox', 'success_first');
select ok((select error_state is null and result is null from a2_results where case_name = 'get_foreign'),
    'A2 foreign merchant GET is indistinguishable from absence');

call pg_temp.a2_capture_get('get_wrong_environment', '80000000-0000-0000-0000-000000000001'::uuid, 'production', 'success_first');
select ok((select error_state is null and result is null from a2_results where case_name = 'get_wrong_environment'),
    'A2 wrong-environment GET is indistinguishable from absence');

-- Wrong execution token cannot mutate.
call pg_temp.a2_capture_resolve(
    'success_resolve_wrong_token', 'success_first', 'success_claim_first',
...
select ok((select error_state is not null from a2_results where case_name = 'success_resolve_wrong_token'),
    'A2 resolution rejects non-current execution token');
select is((select state from app.provider_attempts limit 1), 'executing',
    'A2 rejected resolution token leaves attempt executing');
select is((select collection_status from app.payments limit 1), 'creating',
    'A2 rejected resolution token leaves Payment creating');

-- Successful emulator resolution.
call pg_temp.a2_capture_resolve(
    'success_resolve', 'success_first', 'success_claim_first',
    jsonb_build_object(
        'certainty', 'success',
...
select ok(
    ...
    'A2 success resolution returns pending public Payment with normalized Pix data');
select ok(
    (select not (result ? 'provider') and not (result ? 'providerId') and not (result ? 'providerAccountId')
         and not (result ? 'providerPaymentId') and not (result ? 'providerCost')
         and not (result ? 'routingPolicy') and not (result ? 'executionToken')
       from a2_results where case_name = 'success_resolve'),
...
select ok(
    (select state = 'completed' and http_status_snapshot = 201
         and response_snapshot = (select result from a2_results where case_name = 'success_resolve')
         and completed_at is not null
       from app.request_idempotency where idempotency_key = 'idem-a2-db-success'),
    'A2 success resolution completes idempotency with canonical 201 response snapshot');
...
select ok(
    (select error_state is null and result ->> 'kind' = 'completed'
    ...
    'A2 completed replay returns original completed Payment snapshot');

-- execution_unknown.
call pg_temp.a2_capture_prepare(
    'unknown_first',
    '80000000-0000-0000-0000-000000000001'::uuid,
    'sandbox', 'idem-a2-db-unknown', repeat('e', 64),
    '{"method":"pix","amount":30303,"currency":"BRL","pixExpirationMinutes":30}'::jsonb
);
call pg_temp.a2_capture_claim('unknown_claim', 'unknown_first');
call pg_temp.a2_capture_resolve(
    'unknown_resolve', 'unknown_first', 'unknown_claim',
...
select ok(
    (select state = 'execution_unknown' and last_error_class = 'execution_unknown'
         and recovery_required_at is not null and finished_at is null
       from app.provider_attempts
       where payment_id = (select (result -> 'payment' ->> 'id')::uuid from a2_results where case_name = 'unknown_first')),
    'A2 ambiguous execution becomes execution_unknown with explicit recovery requirement');
select ok(
    (select collection_status = 'creating' from app.payments
       where id = (select (result -> 'payment' ->> 'id')::uuid from a2_results where case_name = 'unknown_first'))
    and
    (select state = 'in_progress' and response_snapshot is null and completed_at is null
...
select is((select result ->> 'kind' from a2_results where case_name = 'unknown_replay'), 'execution_unknown',
    'A2 unknown replay returns same unknown logical Payment without new attempt');

-- Definitive rejection.
call pg_temp.a2_capture_prepare(
    'rejected_first',
    '80000000-0000-0000-0000-000000000001'::uuid,
    'sandbox', 'idem-a2-db-rejected', repeat('f', 64),
    '{"method":"pix","amount":40404,"currency":"BRL","pixExpirationMinutes":30}'::jsonb
);
call pg_temp.a2_capture_claim('rejected_claim', 'rejected_first');
call pg_temp.a2_capture_resolve(
    'rejected_resolve', 'rejected_first', 'rejected_claim',
...
select ok(
    (select state = 'definitively_failed' and last_error_class = 'definitive_rejection'
         and last_error_code = 'emulator_rejected' and finished_at is not null
       from app.provider_attempts
       where payment_id = (select (result -> 'payment' ->> 'id')::uuid from a2_results where case_name = 'rejected_first'))
    and
    (select collection_status = 'failed' from app.payments
       where id = (select (result -> 'payment' ->> 'id')::uuid from a2_results where case_name = 'rejected_first'))
    and
    (select state = 'completed' and http_status_snapshot = 201 and response_snapshot ->> 'status' = 'failed'
...
-- Router ambiguity must fail before new financial state survives.
insert into app.provider_accounts (
    id, provider_id, merchant_id, name, environment, status,
    credentials_ciphertext, capabilities, configuration
) values (
...
call pg_temp.a2_capture_prepare(
    'ambiguous_router',
    '80000000-0000-0000-0000-000000000001'::uuid,
    'sandbox', 'idem-a2-db-ambiguous-router', repeat('1', 64),
...
select ok(
    ...
    and not exists (select 1 from app.request_idempotency where idempotency_key = 'idem-a2-db-ambiguous-router'),
...
select is(
    (select count(*)::text || '|' ||
       ...
       from app.payments),
    '3|0|0|0',
    'A2 success unknown and rejection paths create no ledger job or merchant-webhook effects');
