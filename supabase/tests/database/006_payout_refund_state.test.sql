...
select has_column('app', 'payouts', 'routing_policy_version', 'payout routing-policy snapshot must persist');
...
select has_function('app', 'reserve_payout', array['uuid','text','text','bigint','bigint','jsonb','text','text','text','timestamp with time zone'], 'atomic payout reservation boundary must include routing policy snapshot');
