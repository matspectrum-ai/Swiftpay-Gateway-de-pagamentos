...
select has_index('app', 'accounts', 'accounts_platform_identity_uq', 'platform revenue/expense account identity must be unique');
...
select is(
  (select count(*)::bigint from app.accounts
   where merchant_id = '00000000-0000-0000-0000-000000001001'
     and environment = 'sandbox'
     and currency = 'BRL'
     and account_type = 'merchant_available_liability'),
  1::bigint,
  'ensuring the same merchant account creates exactly one row'
);
...
-- Prepare the four accounts required by the paid-payment posting template.
...
select throws_ok(
  ...
  '23503', null,
  'direct invalid entry is rejected before zero/negative money can persist'
);
