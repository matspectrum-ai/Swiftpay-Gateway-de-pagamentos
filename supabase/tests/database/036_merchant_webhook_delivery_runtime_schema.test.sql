...
select ok(
  coalesce((select prosecdef from pg_proc where oid=to_regprocedure('app.claim_merchant_webhook_deliveries(text,integer,integer)')),false),
  'A4 composed claim is SECURITY DEFINER'
);
select ok(
  coalesce((select prosecdef from pg_proc where oid=to_regprocedure('app.resolve_merchant_webhook_delivery(uuid,uuid,uuid,text,integer,text,text,integer)')),false),
  'A4 composed resolution is SECURITY DEFINER'
);
select is(
  coalesce((select array_to_string(proconfig, ',') from pg_proc where oid=to_regprocedure('app.claim_merchant_webhook_deliveries(text,integer,integer)')),''),
  'search_path=""'::text,
  'A4 composed claim fixes empty search_path'
);
select is(
  coalesce((select array_to_string(proconfig, ',') from pg_proc where oid=to_regprocedure('app.resolve_merchant_webhook_delivery(uuid,uuid,uuid,text,integer,text,text,integer)')),''),
  'search_path=""'::text,
  'A4 composed resolution fixes empty search_path'
);
select is(
  coalesce((select pg_get_function_result(oid) from pg_proc where oid=to_regprocedure('app.claim_merchant_webhook_deliveries(text,integer,integer)')),''),
  'SETOF jsonb',
  'A4 composed claim returns an internal JSON projection set'
);
select is(
  coalesce((select pg_get_function_result(oid) from pg_proc where oid=to_regprocedure('app.resolve_merchant_webhook_delivery(uuid,uuid,uuid,text,integer,text,text,integer)')),''),
  'boolean',
  'A4 composed resolution returns fenced success boolean'
);

select ok(
  case when to_regprocedure('app.claim_merchant_webhook_deliveries(text,integer,integer)') is null then false
       else has_function_privilege('swiftpay_worker',to_regprocedure('app.claim_merchant_webhook_deliveries(text,integer,integer)'),'EXECUTE') end,
  'A4 swiftpay_worker may execute composed webhook claims'
);
select ok(
  case when to_regprocedure('app.resolve_merchant_webhook_delivery(uuid,uuid,uuid,text,integer,text,text,integer)') is null then false
       else has_function_privilege('swiftpay_worker',to_regprocedure('app.resolve_merchant_webhook_delivery(uuid,uuid,uuid,text,integer,text,text,integer)'),'EXECUTE') end,
  'A4 swiftpay_worker may execute composed webhook resolution'
);
select ok(
  case when to_regprocedure('app.claim_merchant_webhook_deliveries(text,integer,integer)') is null then true
        ...
        and not has_function_privilege('public',to_regprocedure('app.claim_merchant_webhook_deliveries(text,integer,integer)'),'EXECUTE') end,
  'A4 composed claims are unavailable to API/Data API/service/public roles'
);
select ok(
  case when to_regprocedure('app.resolve_merchant_webhook_delivery(uuid,uuid,uuid,text,integer,text,text,integer)') is null then true
        ...
        and not has_function_privilege('public',to_regprocedure('app.resolve_merchant_webhook_delivery(uuid,uuid,uuid,text,integer,text,text,integer)'),'EXECUTE') end,
...
select ok(
  not has_function_privilege('swiftpay_worker','app.record_webhook_event(uuid,text,text,text,uuid,text,uuid,text,jsonb,timestamp with time zone)','EXECUTE'),
...
select ok(
  coalesce((
    select count(*)=6 and bool_and(p.oid=any(array[
...
select is(
  coalesce((select count(*)::integer from pg_constraint c join pg_class t on t.oid=c.conrelid join pg_namespace n on n.oid=t.relnamespace where n.nspname='app' and t.relname='webhook_deliveries' and c.conname='webhook_deliveries_signing_secret_version_ck'),0),
  1,
  'A4 signing-secret-version check has one canonical named constraint'
);
select is(
  coalesce((select count(*)::integer from pg_constraint c join pg_class t on t.oid=c.conrelid join pg_namespace n on n.oid=t.relnamespace where n.nspname='app' and t.relname='webhook_deliveries' and c.conname='webhook_deliveries_last_error_code_length_ck'),0),
  1,
