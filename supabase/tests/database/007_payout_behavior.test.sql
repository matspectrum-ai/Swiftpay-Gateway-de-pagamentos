...
select lives_ok($$select app.reserve_payout('00000000-0000-0000-0000-000000007001','sandbox','BRL',6000,500,'{"pix_key":"masked"}'::jsonb,'test-routing-v1','payout-key-1','fp-6000',now())$$,'reservation succeeds when available funds cover gross payout');
select is((select count(*)::bigint from app.payouts),1::bigint,'one payout resource is created');
select is((select routing_policy_version from app.payouts limit 1),'test-routing-v1'::text,'payout snapshots routing policy version at reservation');
...
select lives_ok($$select app.reserve_payout('00000000-0000-0000-0000-000000007001','sandbox','BRL',6000,500,'{"pix_key":"masked"}'::jsonb,'test-routing-v1','payout-key-1','fp-6000',now())$$,'same idempotency key and fingerprint returns existing payout without reposting');
select is((select count(*)::bigint from app.payouts),1::bigint,'idempotent replay creates no second payout');
select throws_ok($$select app.reserve_payout('00000000-0000-0000-0000-000000007001','sandbox','BRL',6000,500,'{"pix_key":"masked"}'::jsonb,'test-routing-v2','payout-key-1','fp-DIFFERENT',now())$$,'23505',null,'same payout idempotency key with different request fingerprint conflicts');
select throws_ok($$select app.reserve_payout('00000000-0000-0000-0000-000000007001','sandbox','BRL',5000,500,'{"pix_key":"masked"}'::jsonb,'test-routing-v1','payout-key-2','fp-5000',now())$$,'23514',null,'insufficient available funds rejects reservation atomically');
