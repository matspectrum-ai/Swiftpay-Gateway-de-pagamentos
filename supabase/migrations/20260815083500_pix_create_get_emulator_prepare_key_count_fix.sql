-- SwiftPay V2 A2: PostgreSQL 17 compatibility fix for Pix prepare pricing validation.
--
-- PostgreSQL 17 does not expose pg_catalog.jsonb_object_length(jsonb). Preserve
-- the frozen contract that the sandbox pricing snapshot contains exactly nine
-- keys by counting jsonb_object_keys instead. No other A2 behavior changes.
...
-- CREATE OR REPLACE preserves the existing execute ACL, but reassert it to keep
-- this corrective migration independently auditable.
