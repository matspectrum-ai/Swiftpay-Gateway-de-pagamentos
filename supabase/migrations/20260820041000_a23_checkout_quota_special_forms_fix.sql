-- SwiftPay V2 A23: preserve A14/A18 quota semantics while using PostgreSQL
-- special forms with native syntax. GREATEST/LEAST/EXTRACT are not
-- schema-qualified pg_catalog functions.
...
begin
  ...
  if v_canonical_request_count < v_limit then
    ...
    v_allowed := false;
    v_remaining := 0;
    v_retry_after_seconds := greatest(1, least(60,
