drop function app.consume_api_abuse_quota(text, text);

create function app.consume_api_abuse_quota(
  p_policy text,
...
begin
  ...
  if p_previous_subject_hash is not null then
    insert into app.api_abuse_windows (
      policy,
      subject_hash,
      window_started_at,
